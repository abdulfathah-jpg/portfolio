...
# **Manual transcription mapping (Malayalam → English)**
PLACE_TRANSCRIPTION = {
    "റിയാദ്": "Riyadh",
    "മക്ക": "Makkah",
    "മദീന": "Madinah",
    "മസ്ജിദുൽ ഹറാം": "Masjid al-Haram",
    "മസ്ജിദുന്നബവി": "Masjid an-Nabawi",
    "സ്വഫ": "Safa",
    "മർവ": "Marwah",
    "ജന്നത്തുൽ ബഖീ": "Jannatul Baqi'",
    "മീഖാത്ത്": "Miqat",
    "കാബാ": "Ka'ba",
    "ഉഹ്ദ്": "Uhud",
    "മസ്ജിദുൽ ഫത്ഹ്": "Masjid al-Fath",
    "മസ്ജിദുൽ ഖിബ്ലതൈൻ": "Masjid al-Qiblatain",
    "മസ്ജിദുൽ ഖുബാഅ്": "Masjid al-Quba'",
}
...
def collect_places(section):
    ...
    for place in section.get("heading_places", []):
        if place in PLACE_COORDS:
            ...
            # Use English transcription if available
            place_en = PLACE_TRANSCRIPTION.get(place, place)
            all_rows.append([
                section_counter,
                section["heading"],
...
for place in route_meta["start_places"]:
    if place in PLACE_COORDS:
        lat, lon = PLACE_COORDS[place]
        place_en = PLACE_TRANSCRIPTION.get(place, place)
        all_rows.insert(0, [0, "START", place_en, lat, lon])
...
for place in route_meta["end_places"]:
    if place in PLACE_COORDS:
        lat, lon = PLACE_COORDS[place]
        place_en = PLACE_TRANSCRIPTION.get(place, place)
        all_rows.append([999, "END", place_en, lat, lon])

# Write CSV
with open("pilgrimage_route_2.csv", "w", encoding="utf-8-sig", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["section_id", "heading", "place_name", "latitude", "longitude"])
    writer.writerows(all_rows)

print("✅ CSV created with English place names: pilgrimage_route_2.csv")
