...
entity_category = "everyday_objects_and_persons"   # change to desired category
...
level1_translations = [
    ...
    "Jannat al-Baqi'", "Mazarat", "Important Adhkars", "Items to Keep in Travel",
    "A Special Hamla for Malayalis"
]
...
# IMPORTANT:
# Force correct X ordering by ZERO-PADDING section numbers -> "01", "02", ..., "20"
# This prevents any lexicographic weirdness in Vega/Altair rendering.
section_order = [f"{int(sec):02d} {label}" for sec, label in zip(level1_section_ids, level1_translations)]

# -----------------------------
# 3. Transliteration maps (as you provided)
# -----------------------------
translit_maps = {
    "place_names": {
...
# -----------------------------
# 4. Select entity category
# -----------------------------
category_data = data[entity_category]
translit_map = translit_maps.get(entity_category, {})

# -----------------------------
# 5. Flatten data into rows (keep your previous robust nested handling)
# -----------------------------
rows = []
for entity_ml, info in category_data.items():
    ...
    # Flatten nested dicts (your previous logic)
...
if df.empty:
    raise ValueError(f"No rows created for '{entity_category}'. Check JSON structure.")

# -----------------------------
# 6. Expand counts → dots with jitter + count for color
#    IMPORTANT: build padded parent_label here (02d) so axis ordering is guaranteed
# -----------------------------
dots = []
for _, row in df.iterrows():
    for _ in range(int(row["count"])):
        ...
        dots.append({
            ...
            "parent_jitter": float(row["parent"]) + float(jitter),
            "parent_label": f"{int(row['parent']):02d} {row['label']}",
            "count_in_section": int(row["count"])
        })

dot_df = pd.DataFrame(dots)

# Discrete bins for strong contrast: 1..5, where 5 means 5+
dot_df["count_bin"] = dot_df["count_in_section"].clip(upper=5).astype(int).astype(str)
...
# -----------------------------
line_df = dot_df.groupby("entity_en", observed=False).filter(lambda x: len(x) > 1)

# -----------------------------
# 9. Summary stats + Option B annotations
# -----------------------------
summary_df = (
    dot_df
    .groupby("entity_en", observed=False)
    .agg(
        total_mentions=("entity_en", "count"),
        sections_covered=("parent", "nunique"),
        first_section=("parent", "min")
    )
    .reset_index()
    .sort_values(by=["sections_covered", "total_mentions"], ascending=False)
)

top_entities = summary_df.head(TOP_N).copy()
rightmost_x = section_order[-1]  # e.g., "20 A Special Hamla..."

text_df = pd.DataFrame({
    "x": [rightmost_x] * len(top_entities),
    "y": top_entities["entity_en"].tolist(),
    "text": [
        f"{row.entity_en} — {int(row.total_mentions)} mentions, {int(row.sections_covered)} sections"
        for _, row in top_entities.iterrows()
    ]
})

# -----------------------------
# 10. Dot raster chart (ORDER FIX: use padded section_order)
# -----------------------------
dots_chart = (
    alt.Chart(dot_df)
    ...
    .encode(
        ...
        y=alt.Y(
            ...
            sort=entity_order,
            axis=alt.Axis(
                title=f"{entity_category.replace('_',' ').title()} (Transliterated)",
                titleFontSize=16, titleFontWeight="bold",
        ...
        tooltip=["entity_ml", "entity_en", "parent_label", "count_in_section"]
    )
)

# -----------------------------
# 11. Lines connecting dots (aligned)
# -----------------------------
lines_chart = (
    alt.Chart(line_df)
...
# -----------------------------
# 12. Text annotation chart (Option B)
# -----------------------------
text_chart = (
    alt.Chart(text_df)
    .mark_text(align="left", dx=10)
    .encode(
        x=alt.X("x:O", sort=section_order),
        y=alt.Y("y:N", sort=entity_order),
        text="text:N"
    )
)
...
# 13. Combine charts
# -----------------------------
final_chart = (dots_chart + lines_chart + text_chart).properties(
    width=2000,
...
# 14. Save chart
# -----------------------------
out_file = f"{entity_category}_dot_raster.html"
final_chart.save(out_file)
print(f"✅ Saved as {out_file} — open in browser to view.")
